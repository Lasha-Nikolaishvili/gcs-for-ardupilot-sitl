#!/usr/bin/env python3
"""
upload_mission_with_telemetry.py

Upload waypoints, geofence barrier points, and rally points to an ArduPilot vehicle
using pymavlink's Mission Protocol—and print live telemetry as it goes.

Usage:
    python upload_mission_with_telemetry.py --connect udp:127.0.0.1:14550
"""
...
class MissionUploader:
    ...
    def _print_telemetry(self, msg):
        ...
        if m == 'HEARTBEAT':
            ...
            print(f"[TELEM] POS   lat={lat:.6f}, lon={lon:.6f}, rel-alt={alt:.2f}m")
        elif m == 'BATTERY_STATUS':
            v = msg.voltages[0] / 1000.0 if msg.voltages[0] > 0 else None
            c = msg.current_battery / 100.0 if msg.current_battery > -1 else None
            r = msg.battery_remaining if msg.battery_remaining > -1 else None
            print(f"[TELEM] BATT  volt={v}V, curr={c}A, left={r}%")
        elif m == 'GPS_RAW_INT':
            print(f"[TELEM] GPS   fix={msg.fix_type}, sats={msg.satellites_visible}")
        # add any other msg types you care about here
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Upload mission + live telemetry via pymavlink')
    parser.add_argument('--connect', required=True,
                        help='e.g. udp:127.0.0.1:14550 or /dev/ttyUSB0,57600')
    args = parser.parse_args()

    uploader = MissionUploader(args.connect)

    # ─── EDIT THESE LISTS ─────────────────────────────────────────────────────────
    waypoints = [
        {'lat': 47.397742, 'lon': 8.545594, 'alt': 10},
        {'lat': 47.397842, 'lon': 8.546594, 'alt': 15, 'hold_time': 2},
        # ...
    ]

    fence_points = [
        {'lat': 47.398000, 'lon': 8.545000},
        {'lat': 47.399000, 'lon': 8.546000},
        {'lat': 47.399500, 'lon': 8.544500},
        # ...
    ]

    rally_points = [
        {'lat': 47.397000, 'lon': 8.545500, 'alt': 5},
        # ...
    ]
    # ────────────────────────────────────────────────────────────────────────────────

    uploader.upload_mission(waypoints)
    uploader.upload_fence(fence_points)
    uploader.upload_rally(rally_points)
    
    uploader.download_and_print_all()
